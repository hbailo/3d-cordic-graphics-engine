...
--! @brief 3D Rotation around the X-axis
--! @details
--! This entity performs a rotation of a 3D vector (xi, yi, zi) around the **X-axis**.
--!
--! The input vector is rotated in the YZ plane by the specified angle (in fixed-point format), leaving the X component unchanged.
--!
...
entity x_axis_rotator is
  ...
  port (
    ...
    --! @brief X-axis rotation angle input (Q0.(N+1) fixed-point)
...
end entity x_axis_rotator;

--! @brief Structural architecture of the X-axis rotator
--! @details
--! This architecture uses the `cordic` entity to rotate the Y and Z components
--! of the input vector in the YZ plane, leaving the X component unchanged.
architecture structural of x_axis_rotator is

begin

  --! @brief YZ-plane rotator (CORDIC core)
  yz_plane_rotator: entity work.cordic
    generic map (
      N => N
    )
    port map (
