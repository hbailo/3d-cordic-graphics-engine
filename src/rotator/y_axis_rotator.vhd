...
--! @brief 3D Rotation around the Y-axis
--! @details
--! This entity performs a rotation of a 3D vector (xi, yi, zi) around the Y-axis.
--!
--! The input vector is rotated in the XZ plane by the specified angle (in fixed-point format), leaving the y component unchanged.
--!
...
entity y_axis_rotator is
  ...
  port (
    ...
    --! @brief Y-axis rotation angle input (Q0.(N+1) fixed-point)
...
end entity y_axis_rotator;

architecture structural of y_axis_rotator is

begin

  --! @brief XZ-plane rotator (CORDIC core)
  xz_plane_rotator: entity work.cordic
    generic map (
      N => N
    )
    port map (
